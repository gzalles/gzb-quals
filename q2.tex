...
%not as important right now, focus on topic
%https://www.aes.org/tmpFiles/elib/20210421/12177.pdf
...
\minipage{0.32\textwidth}
  ...
  \caption{Cardioid Polar Pattern \cite{card_pic}}
  \label{fig:card}
\endminipage\hfill
\minipage{0.32\textwidth}
...
% \todo[inline]{Use better symbols for mics.}

In addition to the five depicted arrays:

\begin{enumerate}
    \item \textbf{Fukada Tree}: top left,%: top left, \cite{fukada2001challenge}.
...
The predominant characteristic of these devices is that proximity between sensors is minimized in order to improve localization estimations and the operating frequency of \textit{virtual microphones} created by combinations of signals. There are three main types of coincident microphone arrays we were able to discern from the literature: \textit{spherical microphone arrays} (SMA), studio microphone arrays, and planar microphone arrays. 
...
%generally we just use 20log10(x).
...
\todo[inline]{WNG math. not as important now, move on. mabande-2009-convex-opt.pdf (eq. 2)}
...
The rest of the paper, talks about four different approaches used for calibrating the capsules which produce our A-format signals. These four approaches are called:
...
\todo[inline]{discrepancy with encoding method. author does not mention pinv() and says filters need to be convolved? was under the impression coefficient were scalars not vectors. middlicott-2020-matlab-tools.pdf post-filtering 2.3.2, great summary.}
...
\todo[inline]{balun converter? dziwis-2019-7th-order-array.pdf}
