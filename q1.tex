...
\subsection{Concert music} \label{subsec:concert-mus}
...
Musil et al. \cite{musil2005library} describes the development of such a project. Despite the shortcomings of the library, the paper does provide some valuable insight into the development of binaural systems:
...
\todo[inline]{I need to better understand condition numbers. I think what I said in the footnote is fine though.}
...
\todo[inline]{Never got an answer from Matthias, Zotter's book has a different weight than the b vector. Pretty sure my interpretation of the eq is correct.}
...
%wrt = with respect to, this is what zotters book calls it.
\end{equation}

\todo[inline]{This is still confusing. If certain parts of squeezed and others stretched doesn't this naturally balance the energy? I think what's happening is that we don't want speakers to overload, so we make things quieter in regions where energy is compressed. One of the papers said the opposite however, that stretching is what cause energy to increase??? Ask Zotter. Ps. see quals/img for photos of hand-written math. Might convert to latex later. Perhaps as appendix info.}
...
Equations \ref{eq:warp-angle-2} and \ref{eq:warp-gain-2} describe the angle and gain modifications respectively \cite{kronlachner2014spatial}. 
...
%license: probably ok to use. Screenshot without permission. The code is GNU so I assume he likes to share. Also, one can download the software so it's irrelevant.
\end{figure}

\paragraph{Widening}

\todo[inline]{I am still reading this paper. It's almost like FM in the ambisonic domain. Kronlachner's thesis has another transform that was not implemented but super interesting.}
...
\subsubsection{Sparta Plug-in Suite}
...
% the equations are a bit complicated, I am just going to describe generally what the papers are proposing.
...
The diffuse-field response criteria is employed using a covariance matrix, which holds variance values for the left and right ear along the main diagonal and true covariance in anti-diagonal elements.

% \subsubsection{York Plug-in Suite}

% \subsubsection{Other Computer Music Languages}

% SuperCollider (SC): 3Dj
% Csound:
% MAX/MSP: ICST
% RTcmix:
% ChucK:
...
%jaroszewicz-2015-spat-cpu-mus
    %cross-synthesis and spectral shredding 
